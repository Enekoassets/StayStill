...
if __name__ == "__main__":
    ...
    # load checkpoint
    train_utils.load_checkpoint(det_config, detail_model, suffix=".min")
    train_utils.load_checkpoint(ctx_config, context_model, suffix=".min")

    if args.index is None:
        # code to execute all transitions at the same time (when --trans is 99)
        if args.trans == 99:
            trans = [5, 15, 30, 45]
            for transition in trans:
                res = det_mdl.eval_on_dataset(
                    det_config, data_loader, detail_model, context_model,
                    transition, args.debug, args.post_processing)
                gpos_loss, gquat_loss, npss_loss = res
                print(det_config["name"])
                print("trans {}: gpos: {:.4f}, gquat: {:.4f}, npss: {:.4f}{}".format(
                    transition, gpos_loss, gquat_loss, npss_loss,
        ...
        json_path_gt = "./json/{}_{}_{}_{}_gt.json".format(
        ...
        json_path = "./json/{}_{}_{}_{}.json".format(
            args.det_config, args.dataset, args.trans, args.index)
