...
def get_interp_loss(dataset, data_loader, context_len, target_idx,
                    mean_rmi, std_rmi, debug=False, quadratic = False):
    """
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        pos_interp, rot_interp = get_interpolated_local_pos_rot(
            positions, rotations, seq_slice, quadratic=quadratic)
...
def get_interpolated_local_pos_rot(positions, rotations, seq_slice, quadratic = False):
    """
...
def get_linear_interpolation(start, end, length, quadratic = False):
    """
...
def quadratic_ease_in_out(tensor, dim=-1):
    # Ensure tensor is float
    tensor = tensor.float()
    
    # Quadratic ease-in-out: f(t) = 2t^2 if t<0.5 else -2(t-1)^2 + 1
    eased = torch.where(
        tensor < 0.5,
        2 * tensor * tensor,
        -2 * (tensor - 1) * (tensor - 1) + 1
    )
    return eased


def _get_interpolated_local_pos_quat(positions, rotations, seq_slice, quadratic = False):
    ...
    root_pos = get_linear_interpolation(
        ...
        positions[..., target_slice, 0, :], n_trans, quadratic=quadratic)[..., None, :]
