...
if __name__ == "__main__":
    ...
    for trans_len in trans_lens:
        ...
        q_interp_pos, q_interp_quat, q_interp_npss = benchmark.get_interp_loss(
            dataset, data_loader, context_len, target_idx, mean_rmi, std_rmi, quadratic=True)

        print(
            "trans: {:2d} \n"
            "zerov_pos: {:.4f}, zerov_quat: {:.2f}, zerov_npss: {:.4f}\n"
            "inter_pos: {:.4f}, inter_quat: {:.2f}, inter_npss: {:.4f}\n"
            "q_inter_pos: {:.4f}, q_inter_quat: {:.2f}, q_inter_npss: {:.4f}".format(
                trans_len, zerov_pos, zerov_quat, zerov_npss,
                interp_pos, interp_quat, interp_npss,
                q_interp_pos, q_interp_quat, q_interp_npss
            )
        )
